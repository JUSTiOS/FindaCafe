...
protocol NearbyCafeUsecaseProtocol {
    func execute(url: String, standard: MyLocationEntity) -> AnyPublisher<[NearbyCafeEntity], Error>
}
...
class NearbyCafeUsecase: NearbyCafeUsecaseProtocol {
    ...
    func execute(url: String, standard: MyLocationEntity) -> AnyPublisher<[NearbyCafeEntity], Error> {
        ...
        return repository.getNearbyCafeList(url: url, longitude: longitude, latitude: latitude)
            .map { $0.documents
                    .map { $0.toDomain() }
